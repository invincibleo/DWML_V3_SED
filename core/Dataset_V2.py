...
class Dataset(object):
    ...
    def get_dataset_file_addr(self):
        file_name = "coding-" + self.FLAGS.coding + "_timeRes-" + str(self.FLAGS.time_resolution) + str(self.dimension) + ".pickle"
    ...
    def get_feature_file_addr(self, sub_dir, data_name):
        hash_parameters = hashlib.sha1((str(self.feature_parameters)+str(self.using_existing_features)+str(self.dimension)).encode('utf-8')).hexdigest()
    ...
    def generate_batch_data(self, category, batch_size=100, input_shape=(1, -1)):
        ...
        while (1):
            ...
            for i in range(0, int(np.floor(num_data_files/batch_size))):
                ...
                labels_for_batch = np.zeros((batch_size, self.num_classes))
                idx = 0
                for key, value in reference_dict.iteritems():
                    ...
                    feature_file_addr = os.path.join(os.path.join(self.dataset_dir, sub_dir), data_name)
                    labels_for_batch[idx, :] = np.array(label_content_list)
                    features, fs = GeneralFileAccessor(file_path=feature_file_addr, mono=True).read()
                    try:
                        features_for_batch[idx, :] = np.reshape(features, (-1, 1))
                    except ValueError:
                        features_for_batch[idx, 0:np.shape(features)[0]] = np.reshape(features, (-1, 1))
                        print("Audio is not 10sec long! Name: " + data_name)
                    idx = idx + 1
